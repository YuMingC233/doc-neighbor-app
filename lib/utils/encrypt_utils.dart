...
class EncryptUtils {
  ...
  static String encrypt(String data) {
    ...
    final encrypter = encrypt_lib.Encrypter(encrypt_lib.RSA(
        publicKey:
            encrypt_lib.RSAKeyParser().parse(publicKey) as RSAPublicKey));
  ...
  static String decrypt(String data) {
    ...
    final encrypter = encrypt_lib.Encrypter(encrypt_lib.RSA(
        privateKey:
            encrypt_lib.RSAKeyParser().parse(privateKey) as RSAPrivateKey));
  ...
  static String decryptWithAes(String encryptedData, encrypt_lib.Key aesKey) {
    ...
    // ECB模式不需要IV，但encrypt库API需要，所以传入空IV
    final decrypted =
        encrypter.decrypt64(encryptedData, iv: encrypt_lib.IV.fromLength(0));
    return decrypted;
  }
  ...
  /// 检查重复提交
  static bool checkRepeatSubmit(String url, dynamic data,
      {int interval = 500}) {
    final requestObj = {
      'url': url,
    ...
    if (_sessionObj == null) {
      ...
      final sUrl = _sessionObj!['url']; // 请求地址
      final sData = _sessionObj!['data']; // 请求数据
      final sTime = _sessionObj!['time']; // 请求时间

      if (sData == requestObj['data'] &&
          (requestObj['time'] as int) - (sTime as int) < interval &&
          sUrl == requestObj['url']) {
        return true;
      } else {
        _sessionObj = requestObj;
